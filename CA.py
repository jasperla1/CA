...
class General_CA:
    def __init__(self):
        ...
        self.cell_rows = 50
        self.states = 2
        self.boundary_conditions = 'dirichlet0' #periodic, dirichlet0, dirichlet1
        self.neighbours = 'square' #1d->(one,two) 2d->(square,hexagon,beehive)
        self.born = [3]
        self.upgrade = []
        self.lives = [2,3]
    ...
    def Generate_Random_Cells(self,percentage):
        try:
            if percentage >= 0 and percentage <= 1:
                ...
                print("percentage needs to be between 0 - 1")
                exit
        except:
            print("percentage needs to be an number between 0 - 1")
            exit
    def Generate_Middle_Cells(self):
        self.Generate_Cells()
        self.cells[0][self.cell_columns//2] = 1
    
    def Generate_Self_Cells(self):
        self.cells = [input().split()]
        for i in range(len(self.cells[0])):
            self.cells[0][i] = int(self.cells[0][i])
...
#-------------------------------Rule-------------------------------------------
    def Temp_Cells_One(self):
        ...
        elif self.boundary_conditions ==  'dirichlet0':
            temp_cells.insert(0,0)
            temp_cells.append(0)
            
        elif self.boundary_conditions ==  'dirichlet1':
            temp_cells.insert(0,1)
            temp_cells.append(1)
        else: 
            print("Give right initial conditions")
            exit
        return temp_cells
...
class Two_Dim(General_CA):
    ...
    def Generate_Self_Cells(self):
        print('Columns:')
        self.cell_columns = int(input())
        print('Rows:')
        self.cell_rows = int(input())
        self.cells = [[]]
        for i in range(self.cell_rows):
            print('Row ',i+1)
    ...
    def Rule(self):
        ...
        if self.neighbours == 'hexagon':
        ...
        elif self.neighbours == 'honeycomb':
            self.states = 2
            ...
            print("Give right neighbourhood")
            exit            
